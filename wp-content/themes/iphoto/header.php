...
<body <?php body_class(); ?>>
	...
	<?php if (is_home()&& $paged<2) { ?>
		<div id="cate" title="<?php echo get_option('iphoto_easing');?>" alt="<?php echo get_option('iphoto_animate');?>" >home</div><!--end cate-->
	<?php } elseif (is_archive()&& $paged<2){?>
		<div id="cate" title="<?php echo get_option('iphoto_easing');?>" alt="<?php echo get_option('iphoto_animate');?>" ><?php $category=get_the_category($post->ID);$name = $category[0]->slug;echo $name;?></div><!--end cate-->
	<?php }?>
	<div id="wrapper">
